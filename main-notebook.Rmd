...
Generiamo i modelli pesati. In questo caso, rimuoviamo il parametro `normalize` dato che stiamo lavorando con una matrice già normalizzata:
```{r}
...
```{r}
binary_matrix <- binarize(rating_matrix, minRating = 4)
```
...
binaryTrainModel <- function(settings) {
 ...
 error <- calcPredictionAccuracy(test, getData(settings$data, "unknown"), given = 60)
 return(error)
}
```
...
```
Facciamo la media e mettiamo tutto in unico data frame:
```{r}
...
E confrontiamo i risultati con un grafico:
```{r}
plot(
