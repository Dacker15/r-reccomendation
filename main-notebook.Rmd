...
All'interno di un sistema di raccomandazione, potremmo anche normalizzare secondo una funzione personalizzata. Come abbiamo visto prima, con l'utilizzo dei timestamp, possiamo ricavare una media pesata in cui diamo meno peso alla valutazione meno recenti ed un peso maggiore alle valutazione più recenti.  
...
#   }
# 
#   normalize_utility <- function(utility_matrix, weights_matrix) {
#   row_means <- rowMeans(utility_matrix, na.rm = TRUE)
#   col_weights <- colSums(weights_matrix)
#   normalized_matrix <- sweep(utility_matrix, 1, row_means, "-")
#   normalized_matrix <- sweep(normalized_matrix, 2, col_weights, "/")
# 
#   
#   means <- NULL
#   weights <- NULL
...
#   x@data <- data
# 
#   x@normalize[[rc]] <- list(method=methods[3],
#     factors=list(means=means))
#   
#   return(x)
# }
```
...
```{r}
# weighted_matrix <- weight_normalize(rating_matrix, timestamp_matrix)
```
